...
--pc-dark: #1A1A2E;       /* Primary text (text-primary). Note: #1A2744 is midnight-navy, used for section dividers */
--pc-slate: #64748B;      /* Secondary text */
--pc-white: #FFFFFF;       /* Card backgrounds, typography */
--pc-bg: #F8F6F3;          /* Page background (warm off-white) */
...
### Documents (Word/PDF)
- **Use the branded template:** When generating DOCX files, reference `templates/passportcard-template.docx` for the standard layout structure (cover page, headers, footers, heading styles, table formatting).
- **Specialized DOCX templates:** For specific document types, use the purpose-built templates in `assets/`:
  - `assets/passportcard-template-meeting-summary.docx` — for meeting summaries
  - `assets/passportcard-template-memo.docx` — for executive memos
  - `assets/passportcard-template-one-pager.docx` — for one-pagers
  - `assets/passportcard-template-research-report.docx` — for research reports
...
- **Use the branded template:** When generating PPTX files, reference `templates/passportcard-template.pptx` for the 10 canonical slide layouts (title, section divider, content text, two-column, icon grid, big number, comparison table, timeline, quote, closing).
...
- Alternating row shading: white / #F8F6F3
...
SparkKit includes a curated image library at `assets/images/`. When generating visual outputs (presentations, prototypes, documents), reference these assets:

### Available Categories
- `assets/images/logos/` — PassportCard logos (full-global, full-israel, icon-logo) in SVG + PNG
- `assets/images/card-product/` — Physical card renders in PNG + SVG
- `assets/images/icons/` — Branded icons (card-in-hand, medal-award, medical-injury, travel-abroad) in SVG + PNG
- `assets/images/lifestyle/` — Professional photos (business-man-abroad, couple-with-newborn, global-citizen, student-abroad, woman-extreme-sports, travel-hero, travel-lifestyle)
- `assets/images/generated/` — AI-generated brand backgrounds and scenes in hi-res (travel, business, health, tech themes)

### Image Selection Guidance
- Board presentations: use generated navy/red backgrounds, lifestyle travel imagery, card product renders
- Product briefs: use icons, card product renders, relevant lifestyle photos
- HTML prototypes: use logos (SVG preferred), icons (SVG preferred), generated backgrounds
- Broker materials: use lifestyle photos, card product renders, generated travel scenes

For full inventory with dimensions and PPTX-readiness status, see `assets/images/powerpoint-ready-inventory.md`.
